...
namespace Log
{
	LOG GetLog(wchar_t logfile[]) //конструктор структуры лог(открывает поток для вывода в файл)
	{
		LOG a = { NULL, NULL };
	...
	void WriteLine(LOG log, char* c, ...)//записывает чаровскую строку
	{//пока не встретили в указателе пустая строка
		char **p = &c;//адрес 1 строки
		while (strcmp(*p, ""))
		{
			*(log.stream) << *p;//вывод в выходной файл
			p++;
		}
	}

	void WriteLine(LOG log, wchar_t* c, ...)//записывает вчар строку(разные типы)
	{
		wchar_t **p = &c;
		char buf[PARM_MAX_SIZE];//для преобр в чар
		while (wcscmp(*p, L""))
		{
			wcstombs(buf, *p, PARM_MAX_SIZE);//преобр в чар
			*(log.stream) << buf;
			p++;
		}
	}
	void WriteLog(LOG log)//чтобы взять текущие дату и время
	{
		struct tm newtime;
		__time64_t ltime;
		errno_t err;
	...
	void WriteParm(LOG log, Parm::PARM parm)//вывод на экран входных параметров для приложения 
	{
		char buf[PARM_MAX_SIZE];
		*(log.stream) << "---- Параметры --------" << endl;
		wcstombs(buf, parm.in, PARM_MAX_SIZE);//преобразует в чар
	...
	void WriteError(LOG log, Error::ERROR error)//вывод в файл или на консоль ошибки открытия
	{
		if (log.stream) //если есть файл то выводим в файл
		{
