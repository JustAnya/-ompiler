...
bool PolishNot(int lextable_pos, LT::LexTable* lextable, IT::IdTable* idtable)
{
	stack<LT::Entry> stk;                                                                //результирующий вектор
	...
	for (int i = lextable_pos; i < lenout; i++)
	{
		...
		if (tmp.lexema == LEX_ID || tmp.lexema == LEX_INT || tmp.lexema == LEX_CHAR || tmp.lexema == LEX_POWR || tmp.lexema == LEX_STRLEN)
		{
			if (idtable->table[tmp.idxTI].id_type == IT::F)//показать Ане------------------------------------------------?
			{
